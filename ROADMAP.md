...
- [~] **Görsel zenginleştirme (sürüyor)** — tümü programatik (Flame/Canvas), ton **sert/gerçekçi** (çamur, toz, is, yıpranma). ① Kamera: tahta sıradaki oyuncuya doğru hafifçe eğilir, sıra değişince yumuşak animasyonla döner (`BoardProjection`, izdüşüm birebir tersinir, testli); piyonlar dik + zemin gölgesi + derinlik ölçeği ✅. ② Zemin dokusu: bir kez bake edilen çamur/benek/krater/yıpranmış ızgara/hedef sektörleri, eğim izdüşümünden geçer ✅. ④ Askerler: "miğferli mevzi" — kazılı toprak taban + miğfer kubbesi (radyal degrade hacim + kenar ışığı) + siper diski; iki taraf aynı form, mavi/kırmızı; sıra vurgusu zeminde nişan reticle'ı ✅. ⑤ Engeller: mayın (yassı zeytin disk + tetik çubukları + amber işaret) · dikenli tel (direkler + sarkan barbed strand'lar); toprak izi; önizleme hayalet ✅. Ek: ayakta duran her şey (asker + engel) sırası gelen oyuncuya doğru **eğimle döner** (`_liftY`); üst üste binme netliği (yassı mayın + kontur halesi); **`hotSeat` param** (varsayılan true; false = AI modu, tahta sabit P1'e bakar) ✅. ③ Çevre/atmosfer: tahtanın etrafındaki siyah boşluk artık bir dünya — `_drawEnvironment` (ekran uzayında, tahtadan önce): ölü kapalı-hava dikey degradesi + ufuk parıltısı; bir kez bake edilen uzak cephe silüeti (`_bakeEnvironment`: alçak sırt hattı + kırık kazık/tel direği saçılması), uzak kenarın arkasına yerleşir ve **eğimle taraf değiştirir** (P2 sırasında 180° dönük, karşı oyuncunun bakışında dik); hafif salınan 2 duman sütunu + yavaş süzülen kül zerreleri (`_Mote`, ortam); yakın kenarda odak dışı siper sırtı (dolu koyu kütle + tepe dudağı + 2 bulanık kazık); tüm viewport'a hafif köşe vinyeti. Tahta eğime göre sırası gelen oyuncunun kenarına doğru kayar (`nearBias`) — siyah boşluk uzak tarafta cepheye toplanır. Yapay zeka modunda (`hotSeat:false`) sabit P1 kenarına yaslanır ✅. ⑥ UI · ⑦ ana menü — sırada.
