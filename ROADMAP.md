...
**⑥ Oyun içi panel + çipler + eylem çubuğu bitti** (commit `1d5a1b7`; kullanıcı: tur sayacına dokunma, paneller + Onayla/Döndür/İptal olsun, sonra hamle animasyonu): `_PlayerPanel` "cephe konsolu" — opak koyu yüzey + hafif degrade + `_TrenchEdge` (tahtaya bakan kenarda tarafın soluk boya bandı + perçin sırası, `CustomPaint`). `_ModeChip` yuvarlak Material → köşeli: seçili amber basılı levha (üst ışık + kalın alt kenar, "saha tabelası" diliyle bir), pasif `surfaceHi` + ince çerçeve. `_FieldButton` (yeni) Döndür/Onayla/İptal aynı dile (hayalet / amber levha / yalnız-ikon). `_ArmoryTag` (yeni). `AppPalette.p1/p2` parlak `#3E6E9E`/`#A2433B` → soluk `#47607A`/`#8A4A3E` (asker miğferleriyle aynı; `_Faction` ile eşleşir). **Tur sayacı (`_TurnTimer`) dokunulmadı.** Tahta hedef-satır tint'leri ve menü `_StandoffMark` bilerek parlak kaldı.

**⑧ Oyun içi ikonlar + hamle animasyonları bitti** (commit `0798e38` + `6fcff6f` + `ef2bf2b`; kullanıcı: cephanelik emojisi kalksın + oyun içi logo, mayın/tel yanında maliyet o logoyla; engel/hareket animasyonsuz "ışınlanma" düzelsin): **İkonlar** — `_SupplyMark` (stilize mermi = "cephane" para birimi; cephanelik etiketi + katlı panel + engel maliyetleri), `_MineMark` (üstten disk + tetik dikenleri), `_WireMark` (direk + tel + X diken); `_ModeChip` `icon`→`mark` builder + `cost:int?` (2. satır: sayı + `_SupplyMark`); etiketler "Mayın · 1"→"Mayın"/"Tel". **Animasyonlar** — `board_component` `_advanceAnimations(dt)` durumu önceki kareyle diff'ler: piyon ~200ms easeOutCubic kayar (+ ekran-y hop; gölge yerde kalır/küçülür), engel `_barrierT` 0→1 ~240ms `_drawBarrierGrowing` ölçekli, `_Dust` adım varışı + engel koymada `_glowBlob` toz (Partiküller kapalıysa yok). `_drawSoldier` artık `Square` yerine animasyonlu `Offset` alır; zemin nişanı da kayan piyonu izler. Perf: kare başı birkaç lerp + ~10 `_glowBlob` (ağır maliyetler önceki commitlerde çözüldü). **Sıradaki:** Faz 2 — yapay zeka rakibi (kullanıcı önce bir prompt verecek).
...
- [x] **Nasıl Oynanır ekranı** — ana menüde sol üst "?" (`Icons.help_outline`) → `HowToPlayScreen` (`lib/ui/how_to_play_screen.dart`): AMAÇ / SIRA / HAREKET / ENGELLER / SÜRE / GENİŞ ARAZİ, `docs/rules.md`'nin kısa oyuncu özeti; simgeler `lib/ui/game_marks.dart`'a taşındı (public `SupplyMark`/`MineMark`/`WireMark`). Commit `0ad0e47`
- [x] **Geniş Arazi modu** — ikinci arazi tipi: 9×9 tahta, oyuncu başına 11 kredi, oyun başında 2–4 rastgele **ağaç** (kare-kapatan engel, §2.1 — asker giremez/atlayamaz, BFS duvar sayar; hedef satırı + bir önü hariç; her `restart`'ta yeni düzen), **karlı savaş alanı teması** (tam yeniden kaplama: zemin dokusu + uzak cephe + kül yerine kar yağışı + soğuk palet). `game_core`: `BoardState.obstacles` + `GameConfig.wideTerrain` + `ObstacleField.roll`; `Pathfinding`/`Rules` engel karelerini duvar sayar. Menüde ayrı "GENİŞ ARAZİ" bölümü (İki Kişilik + Kolay/Orta/Zor); AI 9×9'da bütçe 350ms. Commit zinciri `a0f4751` → `0cc6a6b` → `a01e88e` → `1b22026` → `e94dec8`
